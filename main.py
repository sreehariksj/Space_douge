...
def main():
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT and player.x - PLAYER_VOL >= 0:
                run = False
                break
        ...
        if keys[pygame.K_LEFT] and player.x - PLAYER_VOL >= 0:
            player.x -= PLAYER_VOL
        if keys[pygame.K_RIGHT] and player.x + PLAYER_VOL + PLAYER_WIDTH <= WIDTH:
            player.x += PLAYER_VOL
        ...
        for star in stars[:]:
            ...
            if star.y > HEIGHT:
                stars.remove(star)
            elif star.y + star_hight >= player.y and star.colliderect(player):
                stars.remove(star)
                hit = True
                break
